...
class Auth extends \zp\Auth
{
     ...
     * @return  boolean
     */
    public function ukeylogin($username, $token, $keeptime = 0)
    {
        $admin = Admin::get(['username' => $username, 'token' => $token]);
        if (!$admin) {
            $this->setError('账户不存在');
            return false;
        }
        if ($admin['status'] == 0) {
            $this->setError('账户被禁用了');
            return false;
        }
        if ($admin->loginfail >= 10 && time() - $admin->logtime < 86400) {
            $this->setError('账户被锁定，请24小时后再试');
            return false;
        }
        $admin->loginfail = 0;
        $admin->logtime = time();
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if (!$admin) {
            $this->setError('用户名输入错误');
            return false;
        }
        if ($admin['status'] == 0) {
            $this->setError('您的账号被禁用了');
            return false;
        }
        if ($admin->loginfail >= 10 && time() - $admin->logtime < 86400) {
    ...
    {
        return parent::check($name, $this->id);
    }
...
//        }
    ...
    public function getChildrenAdminIds($withself = false)
    {
        ...
        if (!$this->isSuperAdmin()) {
            ...
            $childrenAdminIds = Admin::where('admin_id', '=', $this->id)->column('id');
        } else {
            //超级管理员拥有所有人的权限
    ...
    /**
     * 获取左侧导航菜的数据
     * @param $list array 菜单数组数据
     * @param $pid int 父ID
     */
    public function getLeftMenu(&$list, $pid = 0)
    {
        $menus = [];
        $n=0;
        foreach ($list as $k => $v) {
            if ($v['pid'] == $pid) {
                $menus[$n] = $v;
                unset($list[$k]);
                $menus[$n]['children']=$this->getLeftMenu($list, $v['id']);
                $n++;
            }
        }
        return $menus;
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        //collection();
        // 必须将结果集转换为数组
        $ruleList = \app\common\model\AuthRule::where('status', 1)
            ->where('is_menu', 1)
            ->order('sort', 'asc')
            ->cache("__menu__")
            ->select()->toArray();

        $indexRuleList = \app\common\model\AuthRule::where('status', 1)
            ->where('pid', 0)
            ->where('is_menu', 1)
            ->column('id,href,pid');
        ...
        foreach ($ruleList as $k => &$v) {
            if (!in_array('*', $userRule)) {
                if (!in_array($v['id'], $userRule)) {
                    unset($ruleList[$k]);
                    continue;
                }
                $indexRuleId = $v['id'];
                if (isset($indexRuleList[$indexRuleId]) && !in_array($indexRuleId, $userRule)) {
                    unset($ruleList[$k]);
                    continue;
                }
            }
            $v['icon'] = ' icon ' . $v['icon'];
            $v['url'] = '/' . $module . '/' . $v['href'];
            $v['badge'] = isset($badgeList[$v['href']]) ? $badgeList[$v['href']] : '';
            $v['py'] = $pinyin->abbr($v['title'], '');
            $v['pinyin'] = $pinyin->permalink($v['title'], '');

            $selected = $v['href'] == $fixedPage ? $v : $selected;
        ...
        $menu = $nav = '';
        if (Config::get('zpadmin.multiplenav')) {
            $topList = [];
            foreach ($ruleList as $index => $item) {
                if (!$item['pid']) {
                    $topList[] = $item;
